...
export class WispWebSocket extends EventTarget {
  ...
  init_connection() {
    ...
    if (!this.connection) {
      ...
      this.connection.onopen = () => { this.init_stream(); };
      this.connection.onclose = () => { this.on_conn_close() };
      this.connection.onerror = () => { this.on_conn_close() };
  ...
  send(data) {
    ...
    if (data instanceof Uint8Array) {
      ...
      data.arrayBuffer().then(array_buffer => { this.send(array_buffer); });
      return;
  ...
  get protocol() { return "binary"; }
  get readyState() { return this._ready_state; }
}
